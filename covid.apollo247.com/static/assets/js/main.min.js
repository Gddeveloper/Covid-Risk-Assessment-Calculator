...
const protocol=window.location.protocol;const hostname=window.location.host;const domain=protocol+"//"+hostname;const url=domain+"/"+"risk";function initBot(){inputObj={age:"",temperature:"",temperatureText:"",gender:"",genderText:"",symptoms:"",symptomsText:"",additionalSymptoms:"",additionalSymptomsText:"",exposureHistory:"",exposureHistoryText:"",underlyingConditions:"",underlyingConditionsText:"",progress:"",progressText:""};botui.message.bot({delay:500,loading:!0,content:"Hi! Our coronavirus disease self assessment scan has been developed on the basis of guidelines from the WHO and MHFW, Government of India. This interaction should not be taken as expert medical advice. Any information you share with us will be kept strictly confidential."}).then(()=>{return botui.message.bot({delay:200,content:"How old are you?"})}).then(()=>{return botui.action.text({cssClass:"form-control",action:{sub_type:"number",placeholder:"Your age in years",required:!0,min:"0",max:"100",button:{label:"Done"}}})}).then(res=>{inputObj.age=res.value;return botui.message.bot({delay:500,loading:!0,content:"Please select your gender"})}).then(()=>{return botui.action.button({delay:100,action:[{text:"Male",value:"1"},{text:"Female",value:"2"},{text:"Others",value:"0"}]})}).then(res=>{inputObj.gender=res.value;inputObj.genderText=res.text;return botui.message.bot({delay:500,loading:!0,content:"Please let us know your current body temperature in degree Fahrenheit (Normal body temperature is 98.6°F):"})}).then(()=>{return botui.action.button({delay:100,action:[{text:"Normal (96°F-98.6°F)",value:"0"},{text:"Fever (98.6°F-102°F)",value:"1"},{text:"High Fever (>102°F)",value:"2"},{text:"Don’t know",value:"0"}]})}).then(res=>{inputObj.temperature=res.value;inputObj.temperatureText=res.text;return botui.message.bot({delay:500,loading:!0,content:"Are you experiencing any of the symptoms below (mark all those applicable)"})}).then(()=>{return botui.action.button({delay:100,cssClass:"multiselect",multiselect:!0,action:[{text:"Dry Cough",value:"0",actionStop:!0,event:function(event,button){event.currentTarget.classList.toggle("active")}},{text:"Sneezing",value:"2",actionStop:!0,event:function(event,button){event.currentTarget.classList.toggle("active")}},{text:"Sore Throat",value:"3",actionStop:!0,event:function(event,button){event.currentTarget.classList.toggle("active")}},{text:"Weakness",value:"4",actionStop:!0,event:function(event,button){event.currentTarget.classList.toggle("active")}}]})}).then(res=>{inputObj.symptoms=res.value.join("|");inputObj.symptomsText=res.text;if(inputObj.temperature==="1"){inputObj.symptoms=inputObj.temperature+(inputObj.symptoms!==""?"|":"")+inputObj.symptoms;inputObj.symptomsText=inputObj.temperatureText+(inputObj.symptoms!==""?", ":"")+inputObj.symptomsText}
...
return botui.message.bot({delay:500,loading:!0,content:"Please select your travel and exposure details"})}).then(()=>{return botui.action.button({delay:100,action:[{text:"No Travel History",value:"0"},{text:"No contact with anyone with Symptoms",value:"1"},{text:"History of travel or meeting in affected geographical area in last 14 days",value:"2"},{text:"Close Contact with confirmed COVID in last 14 days",value:"3"}]})}).then(res=>{inputObj.exposureHistory=res.value;inputObj.exposureHistoryText=res.text;return botui.message.bot({delay:500,loading:!0,content:"Do you have a history of any of these conditions (mark all those applicable)"})}).then(()=>{return botui.action.button({delay:100,multiselect:!0,cssClass:"multiselect",action:[{text:"Diabetes",value:"0",actionStop:!0,event:function(event,button){event.currentTarget.classList.toggle("active")}},{text:"High Blood Pressure",value:"1",actionStop:!0,event:function(event,button){event.currentTarget.classList.toggle("active")}},{text:" Heart Disease",value:"2",actionStop:!0,event:function(event,button){event.currentTarget.classList.toggle("active")}},{text:"Kidney Disease",value:"3",actionStop:!0,event:function(event,button){event.currentTarget.classList.toggle("active")}},{text:"Lung disease",value:"4",actionStop:!0,event:function(event,button){event.currentTarget.classList.toggle("active")}},{text:"Stroke",value:"6",actionStop:!0,event:function(event,button){event.currentTarget.classList.toggle("active")}},{text:"Reduced Immunity",value:"7",actionStop:!0,event:function(event,button){event.currentTarget.classList.toggle("active")}}]})}).then(res=>{inputObj.underlyingConditions=res.value.join("|");inputObj.underlyingConditionsText=res.text;if(inputObj.symptoms===""&&inputObj.additionalSymptoms==="")
return;return botui.message.bot({delay:500,loading:!0,content:"How have your symptoms progressed over the last 48 hrs?"})}).then(()=>{if(inputObj.symptoms===""&&inputObj.additionalSymptoms==="")
return;return botui.action.button({delay:100,action:[{text:"Improved",value:"0"},{text:"No Change",value:"1"},{text:"Worsened",value:"2"},{text:"Worsened Considerably",value:"3"}]})}).then(res=>{inputObj.progress=res?res.value:"0";inputObj.progressText=res?res.text:"Improved";showScore()})}
function showScore(){let body=document.getElementsByTagName("body")[0];body.classList.add("loading");return new Promise(function(resolve,reject){fetch(url,{method:"POST",headers:{"Content-Type":"application/json;charset=utf-8"},body:JSON.stringify(inputObj)}).then(res=>{if(!res.ok){throw res};return res.json()}).then(response=>{setTimeout(()=>{body.classList.remove("loading");window.location.href=domain+"/info?id="+response.score;resolve(response)},2000)}).catch(function(error){body.classList.remove("loading");reject(error)})})}
initBot()
